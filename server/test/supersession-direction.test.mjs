...
//
// Seeded RED on purpose: `resolveSupersessionDirection` does not exist on base. The
...
test('E1: stored newer than incoming -> abstain with direction stored-newer, judge not consulted', async () => {
  const { r, judge } = await evaluate({ olderTruth: { valid_from: LATER }, newerTruth: { valid_from: PAST, assertedAt: LATER } });
  assert.deepEqual(r, { ...ABSTAIN, direction: 'stored-newer' });
  assert.equal(judge.calls.length, 0);
});

test('E2: stored has no truth time -> abstain with direction ambiguous, judge not consulted', async () => {
  const { r, judge } = await evaluate({ olderTruth: {}, newerTruth: { valid_from: PAST, assertedAt: T0 } });
  assert.deepEqual(r, { ...ABSTAIN, direction: 'ambiguous' });
  assert.equal(judge.calls.length, 0);
});

test('E3: stored beyond now + skew -> abstain with direction stored-future, judge not consulted', async () => {
  const { r, judge } = await evaluate({ olderTruth: { valid_from: new Date(8.64e15).toISOString() }, newerTruth: { assertedAt: T0 } });
  assert.deepEqual(r, { ...ABSTAIN, direction: 'stored-future' });
  assert.equal(judge.calls.length, 0);
});
...
test('E5: truth objects omitted -> ambiguous, judge not consulted', async () => {
  const { r, judge } = await evaluate({});
  assert.deepEqual(r, { ...ABSTAIN, direction: 'ambiguous' });
  assert.equal(judge.calls.length, 0);
});

test('E6: incoming newer -> judged and supersedes, direction incoming-newer', async () => {
...
test('E8: flag off -> direction null', async () => {
  const { r } = await evaluate({ enabled: false, olderTruth: { valid_from: PAST }, newerTruth: { assertedAt: T0 } });
  assert.equal(r.supersede, false);
  assert.equal(r.direction, null);
});

test('E9: unpartitioned -> direction null (R1-B1 untouched)', async () => {
  const { r, judge } = await evaluate({ lane: undefined, persona: undefined, olderTruth: { valid_from: PAST }, newerTruth: { assertedAt: T0 } });
  assert.equal(r.supersede, false);
  assert.equal(r.direction, null);
  assert.equal(judge.calls.length, 0);
});
