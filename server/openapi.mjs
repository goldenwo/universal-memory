...
const SCHEMAS = {
  ...
  CheckpointResponse: {
    description: 'Result of a successful checkpoint (ok:true), a thin-transcript abstention (ok:true, skipped), or soft failure (ok:false).',
    oneOf: [
      {
        type: 'object',
        title: 'CheckpointAbstained',
        description: '#185 thin-transcript abstention: the assembled transcript fell below both admission floors (bytes AND turns), so no summary was generated and nothing was written. Raw captures remain on disk and roll into the next checkpoint.',
        required: ['schema_version', 'ok', 'skipped', 'transcript_bytes', 'transcript_turns', 'duration_ms'],
        properties: {
          schema_version: { type: 'integer', enum: [1] },
          ok: { type: 'boolean', enum: [true] },
          skipped: { type: 'string', enum: ['thin_transcript'] },
          transcript_bytes: { type: 'integer', minimum: 0, description: 'Byte length of the trimmed assembled transcript' },
          transcript_turns: { type: 'integer', minimum: 0, description: 'Count of append-turn headers found in the transcript' },
