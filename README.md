...
- **Authored knowledge that lasts** — structured documents (ADRs, character sheets, hypotheses, goals, strategies) live in plain markdown with frontmatter versioning. Superseded documents are auditable; current ones are surfaced by default. **New in v1.1:** the `/adr` Claude Code skill writes an ADR to `docs/decisions/NNNN-<slug>.md` in the consumer's repo, commits it, and registers the decision atomically with the UM server in one step. Run `/adr "<title>"` from any session.
...
- [MIGRATION.md](MIGRATION.md) — step-by-step upgrade guidance per version transition (v0.3 → v0.4 → v0.5 → v0.6 → v0.7 → v0.8 → v1.0).
