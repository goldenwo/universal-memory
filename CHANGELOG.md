...
- **CI smoke probe now fires.** `.github/workflows/smoke.yml` sets `UM_SMOKE_DEDUP_ON=1` on the smoke step, which gates the S2 dedup probe (two identical `/api/memory_capture` writes → second must return `event=DEDUP_MERGED`). Container reads the new default from `server/.env.example` via `install.sh`, so no env-passthrough is needed at the install step.
...
- **New flag `UM_DEDUP_EMBEDDING_THRESHOLD` (default `0.95`)** for the Layer 2 cosine similarity threshold. Conservative default favors precision (fewer false-merges) over recall.
