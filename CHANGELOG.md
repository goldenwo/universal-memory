...
- **UM speaks the mem0 Platform HTTP dialect behind `UM_MEM0_COMPAT_ENABLED` (default OFF — ships inert)** ([`server/lib/mem0-compat.mjs`](server/lib/mem0-compat.mjs); advances [#71](https://github.com/goldenwo/universal-memory/issues/71)). Any existing mem0 Platform client — including the OpenClaw Discord bot's memory plugin in `mode:"platform"` — adopts UM by pointing its `baseUrl` at the server and swapping the API key for `UM_AUTH_TOKEN`; no client changes, no plugin fork. The facade is a dialect adapter over the SAME internals the REST/MCP surfaces call (`umAdd`, search, list/delete) — no second write path — covering the client-pinned route subset: ping, add (`messages[]`→one role-prefixed transcript→single extraction call; `infer:false` verbatim path), v2 search/list with the mem0 filter DSL subset applied facade-side (unknown filter keys fail-loud 400), by-id get/update/delete with a no-leak operator scope check (foreign ids uniformly 404), scoped bulk deletes, single-operator entity projection, and events-as-empty degradation. Auth accepts the mem0 `Token` scheme alongside `Bearer` (same `UM_AUTH_TOKEN`, no loopback bypass); flag-off requests hard-404 BEFORE auth, byte-identical to a server without the feature. Errors speak the client's `{detail}` dialect. Covered by offline route/handler/shape tests plus the S9 smoke round-trip (`UM_SMOKE_MEM0_COMPAT=1`, on in CI: add → search → get → delete → gone, Token + Bearer, wrong-key 401s). Canonical contract, adoption steps (incl. the OpenClaw worked example), oracle pin + reconciliation procedure: [`docs/mem0-compat.md`](docs/mem0-compat.md).
...
### Fixed — CIMD tolerates vendor-declared extra grant types

- **`grant_types` in a client-metadata document are now INTERSECTED with the server's supported set instead of subset-validated** ([`server/lib/oauth/cimd.mjs`](server/lib/oauth/cimd.mjs), [#160](https://github.com/goldenwo/universal-memory/pull/160)). claude.ai's CIMD document added a `jwt-bearer` grant, and the strict subset rule rejected the whole document (`invalid_client` at connect). `authorization_code` is still required, and a `refresh_token`-only document — previously a false-accept — is now rejected. A live-regression test pins claude.ai's exact published document.
...
- **In-band supersession decision** — new `evaluateInBandSupersession()` in [`server/lib/supersede.mjs`](server/lib/supersede.mjs): for a supersede-eligible (flag-on + partitioned) Layer-2 **embedding** dedup hit whose cosine is in the contradiction-overlap band `[UM_DEDUP_EMBEDDING_THRESHOLD, UM_CONTRADICTION_BAND_CEILING]`, a **bounded inline judge call** (`judgeContradiction`) decides whether the "duplicate" is actually a contradiction. The judge fires ONLY for that narrow slice — flag-off, unpartitioned, out-of-band, and Layer-1 hash hits (exact text → never a contradiction) short-circuit before any judge call.
- **Load-bearing invariant (ADR-0007 line 79):** on a confirmed contradiction the keep-older merge is skipped and the newer fact **falls through to the normal upsert as its own `status:current` point**, then [`supersedePoint`](server/lib/supersede.mjs) demotes the older one. Skipping the merge alone is necessary but **not sufficient** (supersession only demotes the older; it never upserts the newer). Wired at the `umAdd` Layer-2 hit in [`server/lib/add.mjs`](server/lib/add.mjs).
- **Crash-safe + fail-soft.** Ordering is **upsert-newer-first, then demote-older**, so a crash in between leaves two `status:current` points (the accepted D1 keep-both trade-off), never the "no current fact" recall-loss. Demotion is fail-soft: if the demote `setPayload` throws, the newer is already current and the older is left for the session-end detector — no silent data-loss path; the user write never fails.
- **New env `UM_CONTRADICTION_BAND_CEILING`** (default `0.87`, the top of the D3.3-measured contradiction cosine span on the same embedder). A **cost bound** (how often the inline judge fires), not a correctness gate — the judge (`UM_AUTOSUPERSEDE_THRESHOLD`, 0.80) is the precision gate. Re-eval if the embedding model changes.
- **DRY single-source** — the `UM_AUTOSUPERSEDE_ENABLED` opt-out predicate (previously inlined in [`checkpoint.mjs`](server/lib/checkpoint.mjs) + [`mem0-mcp-http.mjs`](server/mem0-mcp-http.mjs)) is extracted to `isAutoSupersedeEnabled()` in [`supersede.mjs`](server/lib/supersede.mjs); both existing gates now call it (behavior-identical). Observability: new `um_inband_supersede_total{outcome=superseded|declined|demote_error}` counter.
- **Tests** — new [`server/test/supersede.test.mjs`](server/test/supersede.test.mjs) (decision-surface unit tests + flag/threshold/ceiling drift gates); 8 `umAdd` integration tests in [`server/test/add.test.mjs`](server/test/add.test.mjs) covering the invariant, crash-safe ordering, fail-soft demotion, and the unchanged keep-older paths (out-of-band / flag-off / unpartitioned / hash-hit / judge-declines); registry-count gate updated 13 → 14. Full suite 922 pass / 0 fail / 13 pre-existing skips.
...
Additive metadata partition for qdrant writes — `lane` (topic-area, e.g. `work`, `personal`) and `persona` (identity-aspect, e.g. `me-engineer`) join `project` as orthogonal filter dimensions. Both are slug-shaped (validated by F1's canonical `PROJECT_SLUG_RE`), single-valued, optional. Direct advance of [#72](https://github.com/goldenwo/universal-memory/issues/72) **axis 5 (auto context routing)** — D2 ships the **schema substrate**; D3 will ship the LLM auto-classifier that populates the fields. Builds on F1 (project soft-default, PRs #78–#80) and D1 (cross-surface dedup, PRs #75–#77).

- **Validator `validateLanePersonaSlug({ value, fieldName })`** in [`server/lib/default-project.mjs`](server/lib/default-project.mjs) — reuses `PROJECT_SLUG_RE` since lane and persona share the slug shape with project. Returns `undefined` for unset arms (undefined / null / empty / whitespace-only); throws `INPUT_INVALID`-enveloped errors for non-string + regex-mismatch inputs.
- **Payload schema additions** in [`server/lib/add.mjs`](server/lib/add.mjs) — `umAdd()` stages `metadata.lane` and `metadata.persona` OUT of the `...metadata` spread before payload assembly, validates them, then conditionally re-adds via the same `...(field !== undefined ? { field } : {})` pattern D1 uses for `surfaces`/`projects`. Caller `metadata: { lane: null }` produces a payload with NO `lane` key (anti-goal: omitted = absent payload key, not null/empty/default).
- **uuidv5 seed extension** — when at least one of (lane, persona) is set, the seed appends `:${lane ?? ''}:${persona ?? ''}` after the legacy `${itemHash}:${userId}` shape. When BOTH are unset, the suffix is empty and the seed reduces to the legacy form exactly — pre-D2 dedup-eligible point IDs stay valid. Hash-first H1 colon-safety preserved.
- **Dedup cascade** in [`server/lib/dedup.mjs`](server/lib/dedup.mjs) — new `partitionArm(key, value)` helper builds the conditional `must` filter arm: explicit slug → strict-equality, undefined → `is_empty` predicate (matches points where the field is absent). Layer 1 (content-hash scroll) keeps its `hash` arm; Layer 2 (embedding similarity search) drops the hash arm but applies the same lane/persona partition arms. Net effect: same text + same userId + same (lane, persona) → DEDUP_MERGED; different lane → 2 records (partition guarantee). Legacy points (no lane/persona key) merge symmetrically only with new writes that also omit those fields.
- **MCP write tools** (`memory_capture`, `memory_add`) and REST `/api/add` — `metadata.{lane, persona}` accepted on the input schema. `memory_append_turn` and `memory_checkpoint` are NOT extended (they write vault filesystem only and do not flow through `umAdd()`; out of scope per spec §2). `memory_capture` validates upfront so an invalid slug fails BEFORE the vault write.
- **MCP read tool** (`memory_search`) and REST POST `/api/search` — `filters.{lane, persona}` accepted in the existing `filters` nested object (matches the shipped F1 `filters.project` shape). JS post-filter at `mem0-mcp-http.mjs:~748` (MCP) and `:~2156` (REST POST) AND-combines the new arms with the existing `project` / `type` filters. **Pre-D2 points (no lane/persona payload key) are excluded when an explicit filter is set** — operator-visible new behavior; documented at the schema layer. `memory_list` / `memory_recent` / `memory_state` are NOT extended (vault-backed reads; out of scope per spec §2).
- **Qdrant payload-index** — new [`server/lib/collection-init.mjs:ensurePayloadIndexes`](server/lib/collection-init.mjs) creates indexes on `lane` and `persona` after the boot warmup loop. Idempotent on 409 conflict; WARN-not-throw on other errors so server still boots (filtered queries degrade to full-scan if index creation fails).
- **No env var, no soft-default for lane/persona.** Lane and persona ship coherent from day one — there is no historical heterogeneity to unify, so no `UM_DEFAULT_LANE` equivalent of F1. Omitted = unpartitioned.
- **Out of scope for D2:** LLM auto-classifier (D3); auto-supersession scoped by lane (D3); vault-write tools `memory_append_turn`/`memory_checkpoint` plumbing; vault-frontmatter `lane:`/`persona:` keys; `memory_supersede` / `memory_forget` lane plumbing; B2 `/remember --lane <slug>` flag (separate follow-up — D2 ships dormant for the canonical casual-save path until that flag lands); CC SessionStart-hook lane derivation; backfill sweep for pre-D2 points; `um_writes_total{lane_set, persona_set}` adoption metric; smoke probe + CI wiring; R9 warn-log enrichment (all deferred to hygiene follow-up).
- **Tests** — 23 new across `default-project.test.mjs` (T1–T6 validator unit tests), `add.test.mjs` (T7–T12 + T12b payload-shape and uuidv5 continuity), `dedup.test.mjs` (T13–T18 + T15b cascade end-to-end via the extended mock harness), and new `d2-read-filter.test.mjs` (T19–T22 HTTP-level read-filter integration via the same harness as `mcp-wire-shape.test.mjs`). Mock at `server/test/fixtures/qdrant-mock.mjs` gained an opt-in `honorFilters` flag (~50 LOC) — when set, evaluates `must`-clause filters incl. `is_empty` against pre-seeded results so cascade tests exercise REAL filter behavior, not shape-only assertions.
- **Spec + plan + 3-round paired-Opus review** — `docs/plans/2026-05-14-d2-lane-persona-schema-{spec,plan}.md` (both gitignored). Spec converged to SHIPPABLE at R3 across methodology + operational lenses; 8 BLOCKERs + 15 IMPORTANTs surfaced and resolved across the rounds, primarily corrections to spec assumptions about the codebase (read pipeline reality, uuidv5 formula direction, write-tool count, `createPayloadIndex` site, metadata-spread anti-pattern).

### Added (v1.1) — Phase D3.1: supersession substrate (inert / no behavior change; opt-in only)

**inert / no behavior change; opt-in only.** D3.1 advances [#72](https://github.com/goldenwo/universal-memory/issues/72) **Gap 2 (lane-scoped auto-supersession)** by shipping the supersession substrate. No detector, no LLM, no auto-supersession — the D3.3 flip PR is the sole operator-visible behavior change in the entire arc.

- **`status:'current'`** stamped on new qdrant-fact points by `buildPayload()` in [`server/lib/add.mjs`](server/lib/add.mjs). Pre-D3 points have no `status` field; the existing recall filter already treats absence as `'current'` (absence-tolerant by design — verified, not added).
- **Reserved-field guard extended** in `server/lib/dedup-constants.mjs` — `status`, `supersededBy`, `supersededAt` added to `RESERVED_METADATA_FIELDS`; callers cannot smuggle these via metadata.
- **New `server/lib/supersede.mjs`** — `supersedePoint()` and `unsupersedePoint()` primitives. Callable but not yet invoked by any detector (D3.2 wires them). The `unsupersedePoint()` primitive is the operator undo surface.
- **`unsupersede` action** wired into the `memory_supersede` MCP tool: `{ action: 'unsupersede', id: <qdrant point id> }` flips a superseded point back to `status:'current'`. Inherits the same `isWriteEnabled()` gate and `validateSafeName` id-validation as the existing supersede path.
- **`only_superseded` two-mode listing** on `memory_search` (MCP), `POST /api/search`, and `GET /api/search` (`?only_superseded=true`). Mode (a) partition-scoped (lane/persona filter, MCP + POST only); mode (b) cross-partition all-superseded (GET always mode b). Pagination: `limit` default 50, `offset` default 0; sorted `supersededAt` DESC then `id` ASC. Dormant until a point actually carries `status:'superseded'`.
- **Dedup-layer superseded exclusion** — `checkContentHashDedup` and `checkEmbeddingDedup` in `server/lib/dedup.mjs` exclude `status:'superseded'` candidates. Re-asserting superseded content yields a fresh current point rather than merging into the dead record.
- **Read-filter regression tests** — `test/d3-read-filter.test.mjs` pins default-exclude / `include_superseded` / absence-tolerance behavior; `test/d3-substrate.test.mjs` covers supersede/unsupersede round-trip, reserved-field guard, and dedup exclusion; no existing recall test changed expectations (D3.1 is purely additive).
- **Spec + plan + 5-round paired-Opus review** — `docs/plans/2026-05-16-d3-lane-scoped-auto-supersession-{spec,plan}.md` (both gitignored). Spec converged SHIPPABLE at R4; plan converged at R4. ADR-0006 cross-reference to D3 corrected from `#79` → `#72 Gap 2`.
