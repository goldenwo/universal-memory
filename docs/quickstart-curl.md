...
4. Delegates to `server/install.sh` with any flags you passed

Takes ~3 minutes (mostly the first Docker image pull).

## Flags

Any flag after `--` passes through to `server/install.sh`:

- `--yes` — non-interactive, accept defaults (see `server/install.sh --help`)
