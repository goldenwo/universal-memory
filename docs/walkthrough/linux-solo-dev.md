# Walkthrough: solo developer on Linux

A fresh-eyes walkthrough of installing universal-memory and proving session continuity works. Targeted at a solo developer on Linux (Ubuntu / Debian / Fedora / Arch — any modern distro) with no prior context. **Roughly 10–15 minutes if everything goes right.**
...
> **Status:** Mirror of [`macos-solo-dev.md`](macos-solo-dev.md) authored 2026-05-08, derived from the macOS walkthrough. **Verification level:** macOS-equivalent paths confirmed; Linux-specific commands (GNU `stat`, `systemctl`, etc.) are best-effort and pending a real fresh-eyes runner pass. Paper cuts from a runner land as small follow-up PRs against this doc.
...
| Item | Version / detail |
|---|---|
| Linux | Ubuntu 22.04+ / Debian 12+ / Fedora 39+ / Arch (any with kernel 5.15+) |
| Docker | [Docker Engine 24+](https://docs.docker.com/engine/install/) OR [Docker Desktop](https://www.docker.com/products/docker-desktop/) for Linux 4.30+. Confirm the docker daemon is running before Step 2. |
...
If you installed Docker Engine (not Desktop), make sure your user is in the `docker` group so `docker` commands work without `sudo`:

```bash
groups | grep -q docker && echo "in docker group" || echo "add yourself: sudo usermod -aG docker $USER && log out + back in"
```
...
If that prints `Start Docker first`:
- **Docker Desktop:** open the GUI (or `systemctl --user start docker-desktop` on some distros), wait for it to be ready
- **Docker Engine:** `sudo systemctl start docker` (and `sudo systemctl enable docker` to auto-start on boot)
...
The plugin's hooks read these from your shell environment. Add to `~/.bashrc` (bash) or `~/.zshrc` (zsh) — whichever is your login shell:
...
Source the rc or open a new terminal so the exports take effect: `source ~/.bashrc` (or `~/.zshrc`).
...
# (GNU stat — Linux default. Compare to Step 6's mtime; should be more recent.)
```
...
```bash
tree -L 3 ~/.um/vault/
# Install tree if missing: sudo apt install tree (Debian/Ubuntu) / sudo dnf install tree (Fedora) / sudo pacman -S tree (Arch)
```
...
Something else is on port `6335` (or `6333` for Qdrant). Find it: `ss -ltn 'sport = :6335'` (or `lsof -i :6335` if installed). Either stop the conflicting process or override the port via `MEM0_MCP_PORT` in `server/.env` and `UM_ENDPOINT` in your shell.
...
Your user isn't in the `docker` group. `sudo usermod -aG docker $USER && newgrp docker` (or log out and back in).
...
- **Shell rc file:** Linux distros ship bash by default; the marker block targets `~/.bashrc`. If you've switched to zsh (`chsh -s $(which zsh)`), use `~/.zshrc` instead. Fish-shell users will need to translate the export commands manually.
- **`stat` syntax:** `stat -c '%Y %n'` is GNU-style (Linux default). The macOS walkthrough uses `stat -f '%m %N'` (BSD-style).
- **Port-finding:** `ss -ltn` is the modern replacement for `netstat -ltn`; both work on most distros. `lsof -i :PORT` works if installed.
- **Docker rootless:** if you're on a multi-user machine and want rootless Docker, see [Docker's rootless guide](https://docs.docker.com/engine/security/rootless/). The walkthrough assumes the standard daemon-with-`docker`-group setup.
...
- Linux distro-specific friction not covered (e.g. SELinux contexts blocking volume mounts on Fedora) — means this doc needs distro-specific callouts
