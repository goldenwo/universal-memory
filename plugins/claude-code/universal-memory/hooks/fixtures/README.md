...
| `rollout-sample.jsonl` | **Codex CLI rollout, structurally verbatim, content scrubbed** (2026-09-07, codex-cli 0.153.4). Each line mirrors a real record shape from a native `codex exec` rollout on this box: `session_meta`, a developer-role `response_item/message` (hook context), the user-role first-turn injection (three blocks: `<recommended_plugins>`, `# AGENTS.md instructions for …`, `<environment_context>`), user/assistant messages with `input_text`/`output_text` blocks, `reasoning`, `function_call(_output)`, `event_msg` (`task_started`, `agent_message`, `task_complete`), `turn_context`. Eligible lines: 4, 7, 10, 12 (line 12 mixes a `<system-reminder>` block with real text). Exercised by stop.test.sh S29/S30. |
