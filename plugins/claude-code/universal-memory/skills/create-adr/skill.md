...
The frontmatter `status:` field drives recall suppression after `/adr sync`: a bare `Superseded` / `Deprecated` / `Rejected` token removes the ADR from default recall (recoverable for frontmatter-driven suppression — flip it back and re-sync; a point the server demoted out-of-band, one carrying `supersededBy`/`supersededAt`/`invalidated_at`, stays suppressed regardless of frontmatter); recognized bare tokens are `Proposed | Accepted | Superseded | Deprecated | Rejected`, matched case-insensitively, and anything else (including `Superseded by ADR-NNNN` prose) changes nothing and logs a server-side warning. `/adr sync` also carries the authored `supersedes` / `superseded_by` frontmatter into the index as label-only metadata (flow list `["NNNN-slug"]`, block list, or scalar all parse; empty/`null` values are omitted) — these record which decision replaced which and change no recall behavior; lifecycle stays driven solely by `status:`.
