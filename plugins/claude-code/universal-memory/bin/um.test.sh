...
# ─── Setup: stubs in the real bin dir (dispatcher uses SCRIPT_DIR abs paths) ─
# We write stubs alongside the real dispatcher, then clean up at the end.
STUBS_CREATED=()

create_stub_sh() {
  local cmd="$1"
  local target="$SCRIPT_DIR/um-${cmd}.sh"
  cat > "$target" <<'STUBEOF'
#!/usr/bin/env bash
...
STUBEOF
  ...
  chmod +x "$target"
  STUBS_CREATED+=("$target")
}

create_stub_bin() {
  local cmd="$1"
  local target="$SCRIPT_DIR/um-${cmd}"
  # Only create if NOT already existing (um-forget / um-supersede are real binaries)
  if [ ! -f "$target" ]; then
    printf '#!/usr/bin/env bash\necho "um-%s stub called with: $*"\n' "$cmd" > "$target"
    chmod +x "$target"
    STUBS_CREATED+=("$target")
  fi
}
...
cleanup() {
  for f in "${STUBS_CREATED[@]:-}"; do
    rm -f "$f"
  done
}
trap cleanup EXIT

# Create stubs for .sh-suffix subcommands (A.3-A.8 not yet implemented)
for cmd in search state recent list capture tail; do
  create_stub_sh "$cmd"
done

# create_stub_bin for forget/supersede only if not already present
# (they ARE present in A.0, so we skip creating stubs for them)

# fake home (isolates usage log + .um/config)
fake_home=$(mktemp -d)
FAKE_HOME_DIRS=("$fake_home")

# Helper: run dispatcher
run_um() {
  HOME="$fake_home" UM_NO_USAGE_LOG=1 bash "$UM" "$@"
}
...
HOME="$fake_home6b" UM_NO_USAGE_LOG=1 bash "$UM" search logtest 2>/dev/null || true
...
HOME="$fake_home7" bash "$UM" search sentinel-test 2>/dev/null || true
...
out8=$(cd "$fake_wd8" && HOME="$fake_home" UM_NO_USAGE_LOG=1 UM_SERVER_URL=env-value bash "$UM" search foo 2>&1) && rc8=$? || rc8=$?
